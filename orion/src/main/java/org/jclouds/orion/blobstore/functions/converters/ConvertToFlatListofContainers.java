 ...
 * 
 */
public class ConvertToFlatListofContainers implements Function<JsonNode, List<OrionChildMetadata>> {
	 ...
	 */
	@Inject
	public ConvertToFlatListofContainers(ObjectMapper mapper) {
		this.mapper = mapper;
	}
	...
	public List<OrionChildMetadata> apply(JsonNode parentNode) {
		...
		if (parentNode.has(OrionConstantValues.LIST_CHILDREN)
			...
			for (JsonNode childNode : parentNode.get(OrionConstantValues.LIST_CHILDREN)) {
				try {
					OrionChildMetadata childData = this.mapper.readValue(childNode, OrionChildMetadata.class);
